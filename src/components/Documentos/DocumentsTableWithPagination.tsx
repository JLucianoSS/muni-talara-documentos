...
export const DocumentsTableWithPagination = ({ documents: initialDocs, expedientes }: Props) => {
  ...
  const refetch = async (page: number = currentPage) => {
    setLoading(true);
    try {
      const data = await fetchDocumentsClient(page, 10);
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('¿Eliminar este documento? Se moverá a la papelera.')) return;
    setLoading(true);
    try {
        ...
        <table className="w-full border-collapse bg-white shadow-md ">
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan={7} className="p-6 text-center text-gray-500">No hay documentos.</td>
              </tr>
            ) : (
              documents.map((d) => (
