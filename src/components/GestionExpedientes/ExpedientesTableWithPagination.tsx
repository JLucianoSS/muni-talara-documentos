...
export const ExpedientesTableWithPagination = ({ expedientes: initialExpedientes, areas, users }: ExpedientesTableProps) => {
  ...
  const refetch = async (page: number = currentPage) => {
    setLoading(true);
    try {
      const data = await fetchExpedientesWithDocumentsClient(page, 10);
